...
為什麼這個檢查足夠？可以先用直覺理解：若 `first` 與 `second` 都由同一個 Pattern `P` 重複而成，則可以寫成：
...
兩者必然相同。反過來說，若 `first + second == second + first`，代表兩個字串的排列邊界可以互相對齊，它們必定共享某個基本週期。這個性質可視為字串週期與 Border 理論中的一個結果；本章只需要掌握其直覺：串接交換後仍相同，表示兩者可以由同一個基本 Pattern 重複形成。
...
使用 `std::gcd` 時要注意型別：它要求兩個參數都是整數型別，且實務上應避免混用 `int`、`long long` 與 `std::size_t` 造成模板推導或轉型問題。`first.size()` 與 `second.size()` 都回傳 `std::size_t`，因此可以直接使用：
...
若仍習慣用 `int` 版本的 `gcd`，可先轉成 `int`，但必須確認字串長度不超過 `int` 範圍；若長度可能很大，應使用 `std::size_t` 或 `long long` 版本。兩種版本的數學意思相同，差異在型別與可表示範圍。
...
例如，兩個 Pattern 的長度分別為 4 與 6。若各自不斷重複，最早會同時回到起點的總長度是：

```text
lcm(4, 6) = 12
```

這就像兩個循環燈號，一個每 4 秒亮一次，另一個每 6 秒亮一次，12 秒後會再次同時亮起。GCD 問的是「最大共同分割單位」，LCM 問的是「最早共同同步長度」。
...
偶數中只有 2 是質數。若 `n` 為偶數且不等於 2，則它必然有因數 2，可以直接排除。接著只需要檢查 3、5、7、... 這些奇數，迭代次數約可減半。
...
- 我知道 LCM 應先除 GCD 再乘，也能用週期同步例子理解 LCM。
...
- 我知道長度符合不代表內容符合。
- 我能先寫出列舉 Prefix 的直覺解。
- 我能解釋為何共同週期存在時，兩種串接順序相同，也知道串接交換相同代表兩者共享基本週期。
- 我知道最大共同 Pattern 長度是兩個字串長度的 GCD。

#### 質數與因數分解

- 我知道 0、1 與負數不是質數。
- 我能解釋為何只需檢查到平方根，也知道 2 以外的偶數可直接排除。
