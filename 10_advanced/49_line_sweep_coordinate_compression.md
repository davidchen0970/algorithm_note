...
1. Line Sweep（掃描線）： 想像有一條垂直的掃描線，從左到右滑過整個空間（或時間軸）。我們不需要關心 "沒有事情發生" 的空白區域，只需要在 "起點（事件開始）" 與 "終點（事件結束）" 發生時，更新目前的狀態。
2. Coordinate Compression（座標壓縮）： 當座標值極大但數值數量很少時，我們只保留數字之間的相對大小與順序，把疏散的大座標映射到緊密的連續小數字上（例如將 $10^9$ 壓縮成小於 $N$ 的索引值）。
